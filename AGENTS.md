...
## Testing Guidelines
Vitest runs in a `jsdom` environment and only includes `src/**/*.test.ts`. Add or update tests with every behavior change, especially for menu state, document lifecycle, file system access, and native bridge wrappers. Mirror the source filename when possible, for example `src/lib/paste.ts` with `src/lib/paste.test.ts`. Run `npm run test` before opening a PR; use `npm run tauri:dev` for manual verification of macOS-specific flows.
